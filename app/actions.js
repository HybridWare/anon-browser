...
const DEFAULT_CONFIG_FILE_NAME = '.anonrc'
...
export function createActions ({
  ...
  async function onLearMore () {
    await shell.openExternal('https://github.com/HybridWare/anon-browser')
  }
  ...
  async function onCreateBookmark (event, focusedWindow) {
    for (const webContents of getContents(focusedWindow)) {
      ...
      // If testing from source find and use installed Anon location
      const filePath = appPath || process.argv[0]

      const title = webContents.getTitle()
      const shortcutName = sanitize(title, { replacement: ' ' })
      const url = webContents.getURL()
      const description = `Anon Browser - ${url}`
      ...
      const createShortcut = icon => {
        ...
        // TODO: Kyran: Use Anon icon if no icon provided.
        // TODO: Kyran: OSX doesn't have arguments option. See https://github.com/HybridWare/anon-browser/pull/53#issuecomment-705654060 for solution.
