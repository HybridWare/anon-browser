export default async function makeHost (opts = {}) {
    ...
    const hostScheme = finalOpts.scheme
    const hostPort = finalOpts.port
  ...
  //   }
    ...
    return async function handleHost(url, opt) {
      const session = new Request(url.replace(hostScheme, 'http').replace('~', 'localhost:' + hostPort), opt)
